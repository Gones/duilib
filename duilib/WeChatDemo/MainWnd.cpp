...
CStdString CMainWnd::GetSkinFolder()
{
	return CStdString((CPaintManagerUI::GetInstancePath()) + _T("skin\\"));
}

CStdString CMainWnd::GetSkinFile()
{
...
{
	m_pclFriendsList = (CFriendsListUI *)GetPaintManager()->FindControl("Friends");
	...
	SYSTEMTIME stTime;
	GetLocalTime(&stTime);
	strTime.Format("%02d:%02d", stTime.wHour, stTime.wMinute);
...
{
	m_pclContactsList = (CContactsListUI *)GetPaintManager()->FindControl("Contacts");
