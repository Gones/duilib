...
CFriendsListUI::CFriendsListUI(CPaintManagerUI *pclPaintManager)
	: m_pclRoot(NULL)
	, m_iDelayDeltaY(0)
	, m_iDelayNumber(0)
	, m_iDelayLeft(0)
...
bool CFriendsListUI::Add(CControlUI *pclControl)
{
	if (pclControl == NULL)
	{
		return false;
...
bool CFriendsListUI::AddAt(CControlUI *pclControl, int iIndex)
{
	if (pclControl == NULL)
	{
		return false;
...
bool CFriendsListUI::Remove(CControlUI *pclControl)
{
	if (pclControl == NULL)
	{
		return false;
...
bool CFriendsListUI::RemoveAt(int iIndex)
{
...
void CFriendsListUI::DoEvent(TEventUI &stEvent) 
{
...
CNode *CFriendsListUI::GetRoot()
{
	return m_pclRoot;
}

CNode * CFriendsListUI::AddNode(const tagFriendsInfo &stItemInfo, CNode *pclParent)
{
	if (pclParent == NULL)
	{
		pclParent = m_pclRoot;
	...
	{
		pclListElement = static_cast<CListContainerElementUI *>(m_clDlgBuilder.Create(_T("friendlistitem.xml"), (UINT)0, NULL, m_pclPaintManager));
    }
    else 
	{
	...
	CControlUI *pclLogo = (CControlUI *)m_pclPaintManager->FindSubControlByName(pclListElement, "logo");
	if (pclLogo != NULL)
	{
		pclLogo->SetBkImage(stItemInfo.m_strIDIco.c_str());
	}

	CLabelUI *pclName = (CLabelUI *)m_pclPaintManager->FindSubControlByName(pclListElement, "nickname");
	if (pclName != NULL)
	{
		pclName->SetText(stItemInfo.m_strName.c_str());
	}

	CLabelUI *pclTime = (CLabelUI *)m_pclPaintManager->FindSubControlByName(pclListElement, "time");
	if (pclTime != NULL)
	{
		pclTime->SetText(stItemInfo.m_strTime.c_str());
	}
...
void CFriendsListUI::RemoveNode(CNode *pclNode)
{
...
void CFriendsListUI::SetChildVisible(CNode *pclNode, bool bVisible)
{
	if ((pclNode == NULL) || (pclNode == m_pclRoot))
	{
		return;
...
bool CFriendsListUI::IsExpandable(CNode *pclNode) const
{
	if ((pclNode == NULL) || (pclNode == m_pclRoot))
	{
		return false;
