...
void CMenuWnd::Init(CMenuElementUI* pOwner, STRINGorID xml, LPCTSTR pSkinType, POINT point, bool bTrayMenu)
{
	m_BasedPoint = point;
    m_pOwner = pOwner;
    m_pLayout = NULL;
...
void CMenuWnd::Notify(TNotifyUI& msg)
{
	...
	{
		if ((msg.pSender->GetName() == "menu_official")
			|| (msg.pSender->GetName() == "menu_feedback")
			|| (msg.pSender->GetName() == "menu_help"))
		{
			::ShellExecute(NULL, _T("open"), _T("https://github.com/duilib/duilib"), NULL, NULL, SW_SHOWNORMAL);
		}
		else if (msg.pSender->GetName() == "menu_open")
		{
			::SendMessage(m_hParent, WM_SHOWWINDOW, 1, 0);
		}
		else if (msg.pSender->GetName() == "menu_mini2tip")
		{
			::SendMessage(m_hParent, WM_SHOWWINDOW, 0, 0);
		}
		else if (msg.pSender->GetName() == "menu_exit")
		{
			::SendMessage(m_hParent, WM_DESTROY, 0, 0);
		}
	}
}
