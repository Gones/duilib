...
LRESULT CMainFrameWnd::OnTrayCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	case WM_RBUTTONDBLCLK:
		{
			CMenuWnd *pclMenu = new CMenuWnd(m_hWnd);
			CPoint stPoint;

			GetCursorPos(&stPoint);

			STRINGorID clXmlName("traymenu.xml");
			pclMenu->Init(NULL, clXmlName, _T("xml"), stPoint, true);
		}
		break;
	default:
		break;
...
void CMainFrameWnd::Notify(TNotifyUI& msg)
{
	...
	else if (msg.sType == _T("click")) 
	{
		...
		else if (msg.pSender == m_pclSysMenuBtn)
		{
			...
			STRINGorID clXmlName("mainmenu.xml");
			pclMenu->Init(NULL, clXmlName, _T("xml"), stPoint);
		}
		else if (msg.pSender == m_pclSearchBtn)
		{
			m_pclSearchEdt->SetText("暂不支持该功能！");
		}
...
void CMainFrameWnd::CreateTray()
{
	...
	_snprintf_s(m_stTrayData.szTip, sizeof(m_stTrayData.szTip), "%s", "CNTVDemo");

	Shell_NotifyIcon(NIM_ADD, &m_stTrayData);
}

void CMainFrameWnd::ModifyTray(LPCSTR lpcszTitle, LPCSTR lpcszTip)
{
	m_stTrayData.cbSize = sizeof(NOTIFYICONDATA);
	m_stTrayData.hWnd = m_hWnd;
	m_stTrayData.uID = IDR_MAINFRAME;
	...
	m_stTrayData.uCallbackMessage = WM_TRAYICON;
	_snprintf_s(m_stTrayData.szInfoTitle, sizeof(m_stTrayData.szInfoTitle), "%s", ((lpcszTitle != NULL) ? lpcszTitle : ""));
	_snprintf_s(m_stTrayData.szInfo, sizeof(m_stTrayData.szTip), "%s", ((lpcszTip != NULL) ? lpcszTip : ""));
...
void CMainFrameWnd::CreateTopBar()
{
	...
	m_pclTopBarWnd->Create(m_hWnd, "TopBarWnd", WS_POPUP, NULL);
...
void CMainFrameWnd::CreateBottomBar()
{
	...
	m_pclBottomBarWnd->Create(m_hWnd, "BottomBarWnd", WS_POPUP, NULL);
